...
class DropConnectLayer(nn.Module):
    def __init__(self, p, min_num_sampled=1, max_num_droppings=0.5):
        super(DropConnectLayer, self).__init__()
        self.p = p
        self.min_num_sampled = min_num_sampled
        self.max_num_droppings = max_num_droppings
    ...
    def forward(self, x):
        ...
        #
        num_weights = x.numel()
        min_num_kept = num_weights - int(num_weights * self.max_num_droppings)
        min_num_kept = max(min_num_kept, self.min_num_sampled)

        #
        mask = torch.empty_like(x).uniform_()
        sorted_mask = torch.sort(mask.view(-1))[0]
        threshold_index = int(min_num_kept) - 1
...
class FeSTGCN_net(torch.nn.Module):
    def __init__(self, in_channels, embed_channels, out_channels,adj_matrix,adj_period,p):
        ...
        self.decoder_rnn=torch.nn.GRU(self.embed_size,
                          ...
                          num_layers=1,
                          batch_first=True)
        self.dropconnect1 = DropConnectLayer(p)
    ...
    def forward(self, x, edge_index, edge_weight ):
        ...
        h_feature = self.dropconnect1(h_feature)
...
for i in range(20):
    ...
    net = FeSTGCN_net(in_channels=seq_len, embed_channels=32,
                      ...
                      adj_period=adj_period,p=P).to(device)
